...
export default function QuickAdd() {
  ...
  return (
    <form onSubmit={submit} className="mb-6">
      <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white p-1.5 shadow-sm focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-100">
        <input
          value={text}
        ...
        />
        <button type="submit" disabled={busy || !text.trim()} className="btn-primary !rounded-xl !px-4 !py-2">
          {busy ? "…" : "Registrar"}
        </button>
      </div>
      {msg && (
        <p className={`mt-2 px-1 text-sm ${msg.ok ? "text-emerald-600" : "text-red-500"}`}>{msg.text}</p>
      )}
    </form>
  );
}
