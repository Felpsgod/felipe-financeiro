...
export default function LoginPage() {
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      if (code === "auth/popup-closed-by-user") setError("Login cancelado.");
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4">
      <div className="w-full max-w-sm rounded-xl border border-slate-200 bg-white p-6 text-center shadow-sm">
        <h1 className="mb-1 text-2xl font-bold text-emerald-600">Meu Financeiro</h1>
        <p className="mb-6 text-sm text-slate-500">
          Controle seus cartões, financiamentos e pagamentos
        </p>

        <button
          onClick={handleGoogle}
          disabled={busy}
          className="flex w-full items-center justify-center gap-2 rounded-md border border-slate-300 bg-white py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-60"
        >
